...
if ( isset($_SERVER['QUERY_STRING']) ) {
	...
	$clid= mysqli_real_escape_string($ma, isset($_COOKIE['clid']) ? substr(htmlspecialchars($_COOKIE['clid']),0,36) : session_id());
	
	$_SESSION['ref']=$referer;

	$insertSQL = "INSERT LOW_PRIORITY INTO referer (ref, scrname, post, clid, ip, agent, partnerid) VALUES (".
				"'".$referer."',".
				"'".($_SERVER['SCRIPT_NAME'].(strlen($_SERVER['QUERY_STRING'])!=0?"?".mysqli_real_escape_string($ma,$_SERVER['QUERY_STRING']):""))."',".
				"'".(count($_POST)!=0? str_replace(">","",print_r($_POST,1)):"")."',".
				"'".$clid."',".
				"'".$_SERVER['REMOTE_ADDR']."',".
				"'".mysqli_real_escape_string($ma, $useragent)."',".
				intval($partnerid).
				")";
...
if (!isset($_COOKIE['clid']) && !isset($_SESSION['clid_num']) ){
	...
	$numrows_check_client=$check_client_sql->num_rows;
	...
	if ( $clientrow->num_rows==0 && !isset($dont_insert_client) ){
...
// закрытые направления
	...
	if ( isset($_SESSION['AuthUsername']) ) {
		...
		//maildebugger($closed_directions);
	}
// конец закрытые направления
...
function commiss () {
	$closed_exchange=$GLOBALS['closed_exchange'];
	if ( isset($_SESSION['AuthUsername']) ) {
		$select="SELECT clients.id, clients.name, clients.nikname, clients.wmid, clients.email, clients.clid, clients.phone 
				FROM clients WHERE clients.nikname='".$_SESSION['AuthUsername']."' ";
		$query = _query ($select, "function.php 18");
	...
	$query_addon = "SELECT addon.id, addon.currname1, addon.currname2, addon.inactive,
		...
		addon.value, addon.date FROM addon where  clientid=0 and 
						...
						and closed_exch.currout=addon.currname2 and closed_exch.clid='".
						(isset($client['clid'])?$client['clid']:"")."')
