function changeProject(){
  ...
  var pay_attr = project.attr('pay_attr');
  ...
  $('#pay_attr').text(pay_attr);
  $('#gamecurrency_values').text(game_amounts[$("select[name=projectid] option:selected").val()]);
  var amount = $('#amount').val();
  var crs = (course[currency])?course[currency]:1;

  var gameAmount = amount/(rubamount/crs);
  $('#total').text(Math.floor(total*course['USD']/rubamount/2.5));
  var colorGet = (gameAmount<1)?'red':'#000000';
  gameAmount = Math.floor(gameAmount);
...
$('select[name=purse]').bind('change blur',function(){
 ...
 $('#getamount').bind('change keyup blur',function(){
     ...
     var projectcurrency =   project.attr('gamecurrency');
     var rubamount		  =  parseFloat(project.attr('rubvalue'));
     var amount			  =  getamount*rubamount/course[$("select[name=purse] option:selected").attr('currency')];
 ...
 $('#wmgameform').submit(function(){
       if(!mbStr){
         ...
         $('#gameresult').html('<img src="i/ajax-loader.gif" width="16" height="16">  Подождите..').css('color','#000000');
         $.post('gamepay.php',forma+'&ajax=true',function(data){
           if(data['status'] == 1){
             	$('#gameresult').html('Переход на оплату').css('color','#3A8D12');
             	mbStr = true;
             	$('#wmgameform').append('<input name="description" value="Платеж на ник '+data['nick']+' в игру '+$("select[name=projectid] option:selected").text()+'" type="hidden">');
				$('#wmgameform').submit();
             	//return false;
           }else{
