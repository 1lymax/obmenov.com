...
if ($user->auth("adm")==1) {
	...
	if ( isset($_POST['pay_lq']) && $_POST['pay_lq']=='pay' ) {
	// Liqpay
		echo "pay_liqpay";
		require_once($serverroot."siti/lp.class.php");
		if ( isset($_POST['lq_id']) ) {
			$lq_id=$_POST['lq_id'];
		}else {
			$lq_id=$GLOBALS['lq_id'];
		}
		$liqpay=new liqpay($lq_id);
		echo $liqpay->order_pay($_POST['oid'], isset($_POST['currency'])?$_POST['currency']:"");
	}
	...
	// Liqpay
			...
			$liqpay=new liqpay($lq_id);
			$liqpay->order_pay($_POST['oid'], isset($_POST['currency'])?$_POST['currency']:"");
	}
...
$select="select id, name from merch where type='liqpay'";
$query=_query($select,"");
...
if ( isset($_POST['update_order']) && $_POST['update_order'] == 'ok' ) {
...
$query_orders = "SELECT orders.partnerid, sum(orders.summin) as summ_from,  sum(orders.summout) as summ_to, orders.id as id,
				...
				(select type from currency where name=orders.currin) as currintype, orders.purse_other, orders.disc, orders.wmid,
				...
				orders.clid FROM orders, payment WHERE payment.orderid = orders.id";//.$badcondition;
...
$orders = _query($query_limit_orders, '');
//echo $query_orders;
$orderssumm=_query($query_limit_summ, '');
$orderssumm_row=$orderssumm->fetch_assoc();
//echo "2 ".time()."<br />";
if (isset($_GET['total'])) {
  $total = $_GET['total'];
} else {
...
echo "Всего записей: "._query($query_orders,"")->num_rows;
...
<td>ID заявки: <br /><input name="search1id" type="text" value="<?=$searchvalues['search1id']?>"/></td>
  <?php /*?>  <td>Ф.И.О: <br /><input name="search3name" type="text" value="<?=$searchvalues['search3name']?>"/></td>
    ...
    <option <?=$searchvalues['search1currin']=="LQ" ? "selected='selected'" : ""?> value="LQ">Liqpay</option>
    <option <?=$searchvalues['search1currin']=="WM" ? "selected='selected'" : ""?> value="WM">Webmoney</option>
    <option <?=$searchvalues['search1currin']=="RUB" ? "selected='selected'" : ""?> value="RUB">Русские банки</option>
    ...
    <option <?=$searchvalues['search1currout']=="LQ" ? "selected='selected'" : ""?> value="LQ">Liqpay</option>
    ...
    <tr class="td_head">
    <td>№ партнера:<br /><input name="search1partnerid" type="text" value="<?=$searchvalues['search1partnerid']?>"/></td>
   ...
   <?php /* <td>№ счета магазина: <br /><input name="search2id" type="text" value="<?=$searchvalues['search2id']?>"/>
    ...
    <td>Запросный WMID:<br /> <input name="search1wmid" type="text" value="<?=$searchvalues['search1wmid']?>"/>
    </td>
...
<?php $row_orders=$orders->fetch_assoc();?>
...
<table border="0" align="center" width="100%" class="tableborder">
 <tr><td>Сумма входа: <?=$orderssumm_row['summ_from']?></td><td>Сумма выхода: <?=$orderssumm_row['summ_to']?></td></tr>
  ...
  $td_style='td';$i=0;
  	...
  	?>
    <form name="<?php echo $row_orders['orderid'];?>" action="orders.php?<?php echo $queryString_orders;?>#href<?=$row_orders['orderid']?>" method="post">
    ...
    <table>
      ...
      <tr><td>Партнер</td><td align="right"> <?=$row_orders['partnerid']?>&nbsp; </td></tr>
    ...
    <img src="https://stats.wmtransfer.com/Levels/pWMIDLevel.aspx?wmid=<?=$row_orders['wmid']?>&bg=0xF5F5F5" 
      ...
      <tr><td>IP:</td><td align="right"><a href="http://dns.com.ua/whois/?domain=<?=$row_orders['ip'];?>#<?=$row_orders['ip'];?>"><?=$row_orders['ip']?></a></td></tr>
  ...
  <?php if ( $row_out['summ']!='' ) { ?> 
  <tr><td>Сумма:</td><td align="right"> <?=$row_out['summ']." ".$row_out['val']?></td></tr>
  <?php } if ( $row_orders['wmid']!='' ) { ?> 
  ...
  <?php } ?>    
   ...
   <strong>Действия:</strong><br />  
        ...
        <form name="t<?php echo $row_orders['orderid'];?>#href<?=$row_orders['orderid']?>" action="orders.php?<?php echo $queryString_orders;?>" method="post">
        <input type="hidden" name="id" value="<?php echo $row_orders['orderid'];?>" />
   ...
   <?php if ( $row_orders['ordered']==1 && $row_orders['recept']==1 && $row_orders['canceled']==0 ) { ?>
	...
	onclick="mkpay('<?=($row_orders['summout']+$row_orders['discammount']." ".$row_orders['currout_extname'])?>',<?=$row_orders['id']?>);">
      <?php if ( substr($row_orders['currout'],0,2)=="LQ" ) {?>
      с аккаунта 
      <select name="lq_id">
      <?php foreach ($lq_accounts as $key=>$val) { ?>
      <option value="<?=$val['id']?>"><?=$val['name']?></option>
      <?php }
	  reset($lq_accounts);
	  ?>
      </select> В валюте:
      <select name="currency"><option value=""></option><option value="USD">USD</option>
      <option value="RUR">RUR</option><option value="EUR">EUR</option></select>
      <?php } ?>
    </form>
	...
	onclick="mkpay('<?=($row_orders['summout']+$row_orders['discammount']." ".$row_orders['currout_extname'])?> без учета резерва','s<?=$row_orders['id']?>');">
    </form>
    <form method='post' id="paylp_<?=$row_orders['id']?>">
    
	  <input type='hidden' name='pay_mc' value='pay'>
	  <input type='hidden' name='oid' value='<?=$row_orders['orderid']?>'>
      с аккаунта 
      <select name="lq_id">
      <?php foreach ($lq_accounts as $key=>$val) { ?>
      ...
      <select name="currency"><option value="">По умолчанию</option><option value="USD">USD</option>
      <option value="RUR">RUR</option><option value="EUR">EUR</option></select>
	  <input type='button' value='Выполнить платеж <?=$row_orders['currout_extname']?> через Liqpay'
	onclick="mkpay('<?=($row_orders['summout']+$row_orders['discammount']." ".$row_orders['currout_extname'])?> через Liqpay','lp_<?=$row_orders['id']?>');" />
    </form>
    <?php } ?>
