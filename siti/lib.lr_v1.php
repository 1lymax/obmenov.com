...
*/
Class lrWorker{
	protected $URL = 'https://api.libertyreserve.com/xml/';
    public $Error = '';
    ...
    */
    private function createAuthToken(){
        $datePart = gmdate("Ymd");
        $timePart = gmdate("H");    
        $authString = $this->SecWord.":".$datePart.":".$timePart;
    ...
    */
    public function Balance($reqID){
        $req = '<BalanceRequest id="'.$reqID.'"><Auth><ApiName>'.$this->APIName.'</ApiName><Token>'.$this->createAuthToken().'</Token></Auth>';
        $req .= '<Balance><CurrencyId>LRUSD</CurrencyId><AccountId>'.$this->Account.'</AccountId></Balance>';
        $req .= '<Balance><CurrencyId>LREUR</CurrencyId><AccountId>'.$this->Account.'</AccountId></Balance></BalanceRequest>';
        $resp = $this->xmlHttpsReq('balance.aspx','req='.urlencode($req));
    ...
    */
    public function AccountName($reqID,$account){
        $req = '<AccountNameRequest id="'.$reqID.'"><Auth><ApiName>'.$this->APIName.'</ApiName><Token>'.$this->createAuthToken().'</Token></Auth>';
        ...
        $resp = $this->xmlHttpsReq('accountname.aspx','req='.urlencode($req));
        if($resp === false) return false;
        $vals = $this->lr_XML_Parse($resp);
        if($vals===false) return false;
        $return = array();
    ...
    */
    public function History($reqID,$startdate='',$enddate='',$currency='',$corrAccount='',$direction='',$TransferId='',$ReceiptId='',$TransferType='',$Source='', $Anonymous='', $AmountFrom='', $AmountTo='', $PageSize='', $PageNumber='', $returnxml=false){
        $req = '<HistoryRequest id="'.$reqID.'"><Auth><ApiName>'.$this->APIName.'</ApiName><Token>'.$this->createAuthToken().'</Token></Auth>';
        $req .= '<History><CurrencyId>'.$currency.'</CurrencyId><AccountId>'.$this->Account.'</AccountId><From>'.$startdate.'</From><Till>'.$enddate.'</Till>';
        ...
        $req .= '<ReceiptId>'.$ReceiptId.'</ReceiptId><TransferType>'.$TransferType.'</TransferType><Source>'.$Source.'</Source><Anonymous>'.$Anonymous.'</Anonymous>';
        ...
        $resp = $this->xmlHttpsReq('history.aspx','req='.urlencode($req));
        if($resp === false) return false;
    ...
    */
    public function Transfer($reqID='',$Payee,$Currency,$Amount,$Memo='',$TransferID='',$TransferType='transfer',$Anonimous='false'){
		...
		//}
		$reqID=$this->reqID();
        $req = '<TransferRequest id="'.$reqID.'"><Auth><ApiName>'.$this->APIName.'</ApiName><Token>'.$this->createAuthToken().'</Token></Auth>';
        $req .= '<Transfer><TransferId>'.$TransferID.'</TransferId><TransferType>'.$TransferType.'</TransferType><Payer>'.$this->Account.'</Payer><Payee>'.$Payee.'</Payee>';
        $req .= '<CurrencyId>'.$Currency.'</CurrencyId><Amount>'.$Amount.'</Amount><Memo>'.$Memo.'</Memo><Anonymous>'.$Anonimous.'</Anonymous></Transfer></TransferRequest>';
        $resp = $this->xmlHttpsReq('transfer.aspx','req='.urlencode($req));
        if($resp === false) return false;
        $vals = $this->lr_XML_Parse($resp);
        if($vals===false) return false;
        $return = array();
