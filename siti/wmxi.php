...
define("WMXI_URL_CLASSIC_X8", "https://w3s.webmoney.ru/asp/XMLFindWMPurseNew.asp");//"https://w3s.webmoney.ru/asp/XMLFindWMPurse.asp");
...
define("WMXI_URL_CLASSIC_X18", "https://merchant.webmoney.ru/conf/xml/XMLTransGet.asp");
define("WMXI_URL_LITE_X18", "https://w3s.wmtransfer.com/asp/XMLCreatePurseCert.asp");

//define("WMXI_URL_CLASSIC_X19", "https://passport.webmoney.ru/XML/XMLCheckUser.aspx");
define("WMXI_URL_CLASSIC_X19", "https://apipassport.webmoney.ru/XMLCheckUser.aspx");

define("WMXI_URL_LITE_X19", "");
...
class WMXI {
	...
	function __xml($data) {
		...
		foreach($data as $k => $v) {
			...
			//@mail("support@obmenov.com",'',$result);
		}
	...
	function _request($url, $xml) {
		...
		$query="insert into xml(orderid, query, answer, iface) values(".$oid.",'".mysqli_real_escape_string($GLOBALS['ma'], $xml)."','".
        str_ireplace("'",'"',$result)."', ' ')";
		//echo $query;
		$query=_query2($query,"xml");
		return $result;
	}
	...
	# interface X2
	function X2($tranid, $pursesrc, $pursedest, $amount, $period, $pcode, $desc, $wminvid, $onlyauth) {
		$data["reqn"] = $this->_reqn();
		if ($this->classic) { $data["wmid"] = $this->wmid; }
		if ($this->classic) { $data["sign"] = ""; }
		$data["trans"] = array(
		...
		if ($this->classic) {
			$data["sign"] = $this->_sign($data["reqn"].$data["trans"]["tranid"]
													.$data["trans"]["pursesrc"]
													.$data["trans"]["pursedest"]
													.$data["trans"]["amount"]
													.$data["trans"]["period"]
													.$data["trans"]["pcode"]
													.$data["trans"]["desc"]
...
#		$data["testsign"]["plan"] = "<![CDATA[".$data["testsign"]["plan"]."]]>";
	...
	function X18($wmid, $purse, $payment_no) {
		$data["reqn"] = $this->_reqn();
		if ($this->classic) { $data["wmid"] = $this->wmid; }
		if ($this->classic) { $data["sign"] = ""; }
		$data["wmid"] = $wmid;
		...
		if ($this->classic) {
			$data["sign"] = $this->_sign($data["wmid"].$data["lmi_payee_purse"].$data["lmi_payment_no"]);
		}

		$xml = $this->_xml($data);
		$url = $this->classic ? WMXI_URL_CLASSIC_X18 : WMXI_URL_LITE_X18;
		$result = $this->_request($url, $xml);
		return $result;
	}
	
	
	function X19($opertype, $pursetype, $amount, $wmid, $pnomer, $fname, $iname, $card_number, $bank_account, $bank_name,
				 $emoney_name, $emoney_id, $direction, $phone='') {
		$data["reqn"] = $this->_reqn();
		...
		if ($this->classic) {
			$data["sign"] = $this->_sign($data["reqn"].$opertype.$wmid);//.implode("", array_values($data["trans"])));
		}

		$xml = $this->_xml($data,"passport.request");
		//maildebugger(print_r($xml,1));
